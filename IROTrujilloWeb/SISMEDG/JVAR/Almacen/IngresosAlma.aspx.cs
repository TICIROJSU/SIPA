...
public partial class SISMEDG_JVAR_Almacen_IngresosAlma : System.Web.UI.Page
{
	...
	public void CargaInicial()
	{
		...
		{
			//con.Open();
			string qSql = "select MOVNUMERO, ALMCODIORG, es.EESSS, MOVNUMEDCO, MOVFECHREG, MOVTOT, MOVFFINAN, MOVREFE " +
				"from tmovim left join CATEESS es on ALMCODIORG = RIGHT(es.codEESS, 5) " +
				"where MOVCODITIP = 'E' and CODDEP = '018A01' " +
			...
			if (dtDatoDetAt.Rows.Count > 0)
			{
				...
				foreach (DataRow dbRow in dtDatoDetAt.Rows)
				{
					...
					gDetH += "<td class='' style='text-align: left;'>" + dbRow["ALMCODIORG"].ToString() + " - " + dbRow["EESSS"].ToString() + "</td>";
