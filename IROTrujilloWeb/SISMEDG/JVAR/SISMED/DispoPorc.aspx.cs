...
public partial class SISMEDG_JVAR_SISMED_DispoPorc : System.Web.UI.Page
{
    ...
    public static string GetDetDisponibilidad(string vIndicador, string vPeriodo, string vEESS, string vHidetipo)
    {
        ...
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                gDetAtenciones += "<th class=''>Cons Prom</th>";
                gDetAtenciones += "<th class=''>Stk Mes</th>";
                gDetAtenciones += "<th class=''>Cons Max</th>";
                gDetAtenciones += "<th class=''>Cons Min</th>";
                ...
                foreach (DataRow dbRow in dt.Rows)
                {
                    ...
                    gDetAtenciones += "<td>" + dbRow["Precio"].ToString() + "</td>";
                    gDetAtenciones += "<td>" + dbRow["Stock"].ToString() + "</td>";
                    gDetAtenciones += "<td>" + dbRow["ConsProm"].ToString() + "</td>";
                    gDetAtenciones += "<td>" + dbRow["StkMes_2"].ToString() + "</td>";
                    gDetAtenciones += "<td>" + dbRow["CONS_Max"].ToString() + "</td>";
                    gDetAtenciones += "<td>" + dbRow["CONS_Min"].ToString() + "</td>";
                    gDetAtenciones += "</tr>";
                    gDetAtenciones += Environment.NewLine;
                }
    ...
    public static string GetEESS()
    {
        ...
        try
        {
            string qSql = "select RED, PTO_DIGITACION, CODIGO, ESTABLECIMIENTO from SISMED.dbo.DispPorc GROUP BY RED, PTO_DIGITACION, CODIGO, ESTABLECIMIENTO order by ESTABLECIMIENTO ";
            SqlCommand cmd2 = new SqlCommand(qSql, conSAP00i);
            cmd2.CommandType = CommandType.Text; SqlDataAdapter adapter2 = new SqlDataAdapter();
            adapter2.SelectCommand = cmd2;
    ...
    {
        string vPeriodo = txtAnio.Text; string vCodEESS = txtEESS.Text.Substring(0, 5);
        if (vCodEESS == "018A0") { vCodEESS = "018A01"; }
        ...
        try
        {
            ...
            if (vCodEESS == "018A00")
            {
                qSql = "select PERIODO, CODIGO, ESTABLECIMIENTO, " +
                    "DISP * 100 pDISP, NORMO_STK * 100 pOPT, SOB_No_Acep * 100 pSOB, SUB_Critica * 100 pSUB, SR * 100 pSR, [DES] * 100 pDES, " +
                    "Nivel_de_Disponibilidad, DISP_X_RANGO " +
                    "from SISMED.dbo.DispPorc " +
                    "where PERIODO = '" + vPeriodo + "' and CODIGO = '018A00'";
            }
	...
	public void CargaDetDisponibilidad(string vIndicador, string vPeriodo, string vEESS, string vHidetipo)
	{
		SqlConnection conSAP00i = new SqlConnection(ClassGlobal.conexion_SAP00);
		vPeriodo = ClassGlobal.tmpvAnio;
		string vCodEESS = vEESS.Substring(0, 5);
		...
		try
		{
			string qSql = "Select SSO, Establecimiento, Categoria_, COVID_19, CodMed, Medicamento, TIP, Est, PET, PRECIO, [201908], [201909], [201910], [201911], [201912], [202001], [202002], [202003], [202004], [202005], [202006], [202007], [" + vPeriodo + "], format(ConsProm, '##0.00') ConsProm, stk_fin_diario as Stock, format(COALESCE( stk_fin_diario / nullif(TotalConsumo / NULLIF(TotalConteo, 0), 0) , 0), '##0.00') as StkMes_2, CONS_MAX, CONS_MIN " +
				"from (select Disp.*, isnull(tabact.Consumo, 0) '" + vPeriodo + "', (disp.Stock - isnull(tabact.Consumo, 0)) + isnull(tabact.Ingresos, 0) stk_fin_diario, isnull(Disp.TOTAL_CONS, 0) + ISNULL(tabact.Consumo, 0) TotalConsumo, isnull(Disp.CONTEO_CONS, 0) + (case when tabact.Consumo>0 then 1 else 0 end) TotalConteo " +
				"from SISMED.dbo.Disp Disp full outer join " +
				"(select tf.codigo_pre2, sum(tf.Consumo) Consumo, sum(tf.ingre) + sum(tf.reingre) Ingresos, tf.codigo_med, sum(tabstk.stock_fin) stock_fin " +
				"from SISMED.dbo.tformdetcons tf full outer join (select codigo_pre2, periodo, codigo_med, stock_fin " +
			...
			if (dt.Rows.Count > 0)
			{
				gDetAt1 += Environment.NewLine + "<div class='box' style='color:#000000; font-size:20px;'><div class='box-body table-responsive no-padding'>";
				gDetAt1 += "<table class='table table-hover' id='tbldscrg' style='text-align: right; font-size: 14px; '>";
				gDetAt1 += "<th class=''>Nro </th>";
				for (int i = 0; i < dt.Columns.Count; i++)
				{ gDetAt1 += "<th class=''>" + dt.Columns[i].ColumnName + " </th>"; }
				gDetAt1 += Environment.NewLine;
				int nroitem = 0;
				if (dt.Rows.Count > 0)
				{
					foreach (DataRow dbRow in dt.Rows)
					{
						nroitem += 1;
						gDetAt1 += "<tr>";
						gDetAt1 += "<td>" + nroitem + "</td>";
						for (int i = 0; i < dt.Columns.Count; i++)
						{
							gDetAt1 += "<td>" + dbRow[dt.Columns[i].ColumnName].ToString() + "</td>";
						}
						gDetAt1 += "</tr>";
						gDetAt1 += Environment.NewLine;
					}
				...
				//gDetAtenciones += "||sep||";
				///////////////////////////////////////////////////////////////////-*--///     
				gDetAt2 += Environment.NewLine + "<div class='box' style='color:#000000; font-size:20px;'><div class='box-body table-responsive no-padding'>";
				gDetAt2 += "<table class='table table-hover' id='tblbscrJS' style='text-align: right; font-size: 14px; '>";
				gDetAt2 += "<th class=''><button class='btn btn-default' type='button' title='Max.: 3,000 Registros' onclick=\"exportTableToExcel('tbldscrg')\"><i class='fa fa-download '></i></button> </th>";
				gDetAt2 += "<th class=''>Medicamento </th>";
				gDetAt2 += "<th class=''>Precio</th>";
				gDetAt2 += "<th class=''>Stock</th>";
				gDetAt2 += "<th class=''>Cons Prom</th>";
				gDetAt2 += "<th class=''>Stk Mes</th>";
				gDetAt2 += "<th class=''>Cons Max</th>";
				gDetAt2 += "<th class=''>Cons Min</th>";
				gDetAt2 += "</tr>" + Environment.NewLine;
				nroitem = 0;
				...
				foreach (DataRow dbRow in dt.Rows)
				{
					nroitem += 1;
					gDetAt2 += "<tr>";
					gDetAt2 += "<td>" + nroitem + "</td>";
					gDetAt2 += "<td style='text-align: left;'>" + dbRow["Medicamento"].ToString() + "</td>";
					gDetAt2 += "<td>" + dbRow["Precio"].ToString() + "</td>";
					gDetAt2 += "<td>" + dbRow["Stock"].ToString() + "</td>";
					gDetAt2 += "<td>" + dbRow["ConsProm"].ToString() + "</td>";
					gDetAt2 += "<td>" + dbRow["StkMes_2"].ToString() + "</td>";
					gDetAt2 += "<td>" + dbRow["CONS_Max"].ToString() + "</td>";
					gDetAt2 += "<td>" + dbRow["CONS_Min"].ToString() + "</td>";
					gDetAt2 += "</tr>";
					gDetAt2 += Environment.NewLine;
				}

				gDetAt2 += "</table></div></div><hr style='border-top: 1px solid blue'>";
			}
			else
			{
				gDetAt2 = "<table></table>||sep||<table></table><hr style='border-top: 1px solid blue'>";
			}

		}
		catch (Exception ex)
		{
			gDetAt2 += "-" + "-" + ex.Message.ToString();
		}
		LitTABL1.Text = gDetAt2;
		LitTABL1_1.InnerHtml = gDetAt1;
    ...
    protected void btnFiltrar_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            //LitTABL1.Text = "<div style='display: block'>" + qSql + "</div>";
	...
	protected void btnDispCovid_Click(object sender, EventArgs e)
	{
		string vIndicador = "Covid";
		string vPeriodo= txtAnio.Text;
		string vEESS= txtEESS.Text;
		string vHidetipo= HideTipo.Value;
		CargaDetDisponibilidad(vIndicador, vPeriodo, vEESS, vHidetipo);
	}
}