...
public partial class ASPX_ExtIROJVAR_Foto_FOxPacCiego : System.Web.UI.Page
{
    ...
    public void CargaTablaDT()
    {
        ...
        try
        {
            string qSql = "SELECT        tOrden.rFila, tOrden.EESS, PCAUO, COUNT(*) AS Total " +
                "FROM            nueva.dbo.FO " +
                ...
                "GROUP BY PCAUO, tOrden.rFila, tOrden.EESS " +
            ...
            string vGraf1 = "<div id='chartc1' style='height: 500px; width: 100%;'></div> " + Environment.NewLine +
                ...
                "			text: '¿Ciego en al menos un ojo debido a la RD?'  " + Environment.NewLine +
                "		},  " + Environment.NewLine +
            ...
            for (int i = 0; i < dtDet.Rows.Count; i++)
            {
                ...
                if (vNameDXP != dbRow["PCAUO"].ToString())
                {
                    vNameDXP = dbRow["PCAUO"].ToString();
                    if (i > 0) { vGraf1 += " ]  }, " + Environment.NewLine; }
                    vGraf1 += "		{ " + Environment.NewLine +
                        "			type: 'stackedColumn100',  " + Environment.NewLine +
                        "			legendText: '" + dbRow["PCAUO"].ToString() + "',  " + Environment.NewLine +
                        "			name: '" + dbRow["PCAUO"].ToString() + "',  " + Environment.NewLine +
