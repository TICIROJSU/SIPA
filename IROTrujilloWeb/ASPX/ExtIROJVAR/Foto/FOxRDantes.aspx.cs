...
public partial class ASPX_ExtIROJVAR_Foto_FOxRDantes : System.Web.UI.Page
{
    ...
    public void CargaTablaDT()
    {
        ...
        try
        {
            string qSql = "SELECT        tOrden.rFila, tOrden.EESS, EARD, COUNT(*) AS Total " +
                "FROM            nueva.dbo.FO " +
                ...
                "GROUP BY EARD, tOrden.rFila, tOrden.EESS " +
            ...
            string vGraf1 = "<div id='chartc1' style='height: 500px; width: 100%;'></div> " + Environment.NewLine +
                ...
                "			text: '¿Examinado antes para detectar RD?'  " + Environment.NewLine +
                "		},  " + Environment.NewLine +
            ...
            for (int i = 0; i < dtDet.Rows.Count; i++)
            {
                ...
                if (vNameDXP != dbRow["EARD"].ToString())
                {
                    vNameDXP = dbRow["EARD"].ToString();
                    if (i > 0) { vGraf1 += " ]  }, " + Environment.NewLine; }
                    vGraf1 += "		{ " + Environment.NewLine +
                        "			type: 'stackedColumn100',  " + Environment.NewLine +
                        "			legendText: '" + dbRow["EARD"].ToString() + "',  " + Environment.NewLine +
                        "			name: '" + dbRow["EARD"].ToString() + "',  " + Environment.NewLine +
