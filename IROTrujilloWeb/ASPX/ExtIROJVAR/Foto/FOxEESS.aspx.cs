...
public partial class ASPX_ExtIROJVAR_Foto_FOxEESS : System.Web.UI.Page
{
    ...
    public void CargaTablaDT()
    {
        ...
        try
        {
            string qSql = "select tOrden.rFila, tOrden.EESS, DXP_FO as DXP, count(*) as Total from NUEVA.dbo.fo " +
                "left join ( " +
                "	 select ROW_NUMBER() OVER(ORDER BY EST_FO DESC) AS rFila, EST_FO as EESS from NUEVA.dbo.fo " +
                "	 group by EST_FO" +
                ") tOrden on EST_FO = tOrden.EESS " +
                "where FEC_FO >= @vFecDesde and FEC_FO <= @vFecHasta " +
                "group by DXP_FO, tOrden.rFila, tOrden.EESS " +
                "order by DXP_FO ";
            ...
            //    "showInLegend: true, name: 'Atenciones', " +
            //    "dataPoints: " +
            //    "[";
            ...
            //        vGraf1 += "{x: " + i + ", " +
            //            "label: '" + dbRow["EESS"].ToString() + " - DXP: " + dbRow["DXP"].ToString() + " [" + vTotPorc + "%]', " +
            ...
            string vGraf1 = "<div id='chartc1' style='height: 500px; width: 100%;'></div> " + Environment.NewLine +
                ...
                "			text: 'REPORTE DE PACIENTES SEGUN DIAGNOSTICO'  " + Environment.NewLine +
                "		},  " + Environment.NewLine +
            ...
            for (int i = 0; i < dtDet.Rows.Count; i++)
            {
                ...
                if (vNameDXP != dbRow["DXP"].ToString())
                {
                    vNameDXP = dbRow["DXP"].ToString();
                    if (i > 0) { vGraf1 += " ]  }, " + Environment.NewLine; }
                    vGraf1 += "		{ " + Environment.NewLine +
                        "			type: 'stackedColumn100',  " + Environment.NewLine +
                        "			legendText: '" + dbRow["DXP"].ToString() + "',  " + Environment.NewLine +
                        "			name: '" + dbRow["DXP"].ToString() + "',  " + Environment.NewLine +
