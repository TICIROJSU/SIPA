...
public partial class ASPX_ExtIROJVAR_Foto_FOxDXP : System.Web.UI.Page
{
    ...
    public void CargaTablaDT()
    {
        ...
        try
        {
            string qSql = "select tOrden.rFila, tOrden.EESS, SEX_FO as Genero, count(*) as Total from NUEVA.dbo.fo  " +
                "left join (  " +
                ...
                "group by SEX_FO, EST_FO, tOrden.rFila, tOrden.EESS " +
                "order by EST_FO ";
            ...
            string vNameDXP = "vNameGenero_ValorInicialSinSentido";
            for (int i = 0; i < dtDet.Rows.Count; i++)
            {
                DataRow dbRow = dtDet.Rows[i];
                if (vNameDXP != dbRow["Genero"].ToString())
                {
                    vNameDXP = dbRow["Genero"].ToString();
                    if (i > 0) { vGraf1 += " ]  }, " + Environment.NewLine; }
                    vGraf1 += "		{ " + Environment.NewLine +
                        "			type: 'stackedColumn100',  " + Environment.NewLine +
                        "			legendText: '" + dbRow["Genero"].ToString() + "',  " + Environment.NewLine +
                        "			name: '" + dbRow["Genero"].ToString() + "',  " + Environment.NewLine +
