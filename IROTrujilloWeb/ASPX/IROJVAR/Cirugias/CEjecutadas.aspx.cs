...
public partial class ASPX_IROJVAR_Cirugias_CEjecutadas : System.Web.UI.Page
{
    string html = "";
    ...
    public static string GetDetEspecialidadxMes(string vanio, string vmes, string vmesnom)
    {
        ...
        try
        {
            string qSql = "select Especialidad, count(Id_SOP) as Cantidad " +
    ...
    public static string GetDetPacientexEspexMes(string vanio, string vmes, string vmesnom, string vespec)
    {
        ...
        try
        {
            string qSql = "select *, FORMAT(cast(Dia as int), '00') as DiaFormat from Estadistica.dbo.Librosop " +
                "where Ano = '" + vanio + "' and Mes = '" + vmes + "' and Especialidad = '" + vespec + "' " +
                "order by cast(Dia as int), Ap_Paterno";
            SqlCommand cmd2 = new SqlCommand(qSql, conSAP00i);
            cmd2.CommandType = CommandType.Text; SqlDataAdapter adapter2 = new SqlDataAdapter();
            adapter2.SelectCommand = cmd2;
    ...
    public void CargaTablaDTIni()
    {
        ...
        {
            //con.Open();
            string qSql = "select Mes, NUEVA.dbo.fnMesTexto(Mes) as MesNom, count(Id_SOP) as Cantidad " +
                "from Estadistica.dbo.Librosop where Ano = '" + vAnio + "' " +
                "Group by Mes " +
