...
public partial class ASPX_IROJVAR_SISMED2_DispoPorc : System.Web.UI.Page
{
    ...
    public static string GetDetDisponibilidad(string vIndicador, string vPeriodo, string vEESS, string vHidetipo)
    {
        ...
        try
        {
            string qSql = "Select SSO, Establecimiento, Categoria_, COVID_19, CodMed, Medicamento, TIP, Est, PET, PRECIO, [201908], [201909], [201910], [201911], [201912], [202001], [202002], [202003], [202004], [202005], [202006], [202007], [202008], format(ConsProm, '##0.00') ConsProm, stk_fin_diario as Stock, format(COALESCE( stk_fin_diario / nullif(TotalConsumo / NULLIF(TotalConteo, 0), 0) , 0), '##0.00') as StkMes_2, CONS_MAX, CONS_MIN " +
    ...
    protected void btnFiltrar_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            string qSql = "SELECT *, pOPT + pSOB + pSR AS pDISP FROM " +
                ...
                "	sum(case when [DES]=1 then 1 else 0 end) / sum(CANT) * 100 pDES " +
                "FROM SISMED.dbo.Disp " +
                "WHERE CodES='" + vCodEESS + "' AND CodMed IN (select codSISMED from SISMED.dbo.MedTipo where tipo='" + vMedTipo + "')" +
                ") TAB1";
