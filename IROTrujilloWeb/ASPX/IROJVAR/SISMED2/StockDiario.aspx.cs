...
public partial class ASPX_IROJVAR_SISMED2_StockDiario : System.Web.UI.Page
{
	...
	protected void bntBuscar_Click(object sender, EventArgs e)
	{
		...
		{
			//con.Open();
			string qSql = "select Red, Microrred, codDEP, EESSS, count(1) Items, MAX(DATEADD(HOUR, -5, fechCarga)) as FechCarga " +
				"from mstkalmde left join CATEESS on '000' + codDEP = codEESS " +
				"where " + vgWhere + " group by Red, Microrred, codDEP, EESSS; ";
			...
			if (dt.Rows.Count > 0)
			{
				...
				foreach (DataRow dbRow in dt.Rows)
				{
					...
					gDetAtenciones += "<tr onclick=\"jsCargaMedicam('" + dbRow["codDEP"].ToString() + "')\" data-toggle='modal' data-target='#modalstkprod'>";
					gDetAtenciones += "<td>" + nroitem + "</td>";
					gDetAtenciones += "<td style='text-align: left;'>" + dbRow["Red"].ToString() + "</td>";
	...
	public static string GetStkProd(string vEESS)
	{
		...
		try
		{
			...
			if (dtDatoDetAt.Rows.Count > 0)
			{
				...
				gDetstr += "<table class='table table-hover' style='text-align: right; font-size: 14px; '>";
				...
				foreach (DataRow dbRow in dtDatoDetAt.Rows)
				{
					...
					gDetstr += "<td style='text-align: left;' >" + dbRow["Medicamento"].ToString() + "</td>";
