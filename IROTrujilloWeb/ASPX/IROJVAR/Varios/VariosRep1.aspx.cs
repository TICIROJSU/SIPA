...
public partial class ASPX_IROJVAR_Varios_VariosRep1 : System.Web.UI.Page
{
	string html = "";
	...
	public void CargaTablaDTIni()
	{
		...
		try
		{
			...
			string qSql = "Select TabTC.Anio, TabTC.Mes, isnull(TabTC.TAR_CONT, 0) TAR_CONT, isnull(TabTC.CHQ_CONT_NO, 0) CHQ_CONT_NO, isnull(TabU.UPE_CONT, 0) UPE_CONT from " +
                ...
                "		ON RIGHT('000000' + TAR.HIC_TAR, 6) = RIGHT('000000' + FICHAFAM, 6) and TAR.FecCita = HIS.FecHIS " +
				"	) Tab " +
				...
				"	FROM NUEVA.dbo.UPE " +
                "   WHERE YEAR(FEC_UPE)=@Anio AND (MONTH(FEC_UPE) between @desde and @hasta) and PRG_UPE <> '13' " +
			...
			cmd2.Parameters.AddWithValue("@Anio", Convert.ToString(DDLAnio.SelectedValue));
			cmd2.Parameters.AddWithValue("@desde", Convert.ToString(DDLMesDesde.SelectedValue));
			cmd2.Parameters.AddWithValue("@hasta", Convert.ToString(DDLMesHasta.SelectedValue));
			...
			if (dtDato.Rows.Count > 0)
			{
				...
				html += "<caption class='text-center'><b>" + DDLAnio.SelectedItem.Text + "</b></caption>";
				html += "<tr>";
