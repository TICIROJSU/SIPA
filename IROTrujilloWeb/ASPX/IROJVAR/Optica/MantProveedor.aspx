<%@ Page Language="C#" MasterPageFile="../../MasterPage.master" AutoEventWireup="true" CodeFile="MantProveedor.aspx.cs" Inherits="ASPX_IROJVAR_Optica_MantProveedor" %>
   ...
   <title>IRO - HISMINSA</title>
    ...
    <script language="javascript" type="text/javascript">
		...
		function btnAgregar() {
            ...
            params.vNDoc = document.getElementById("<%=txtRUC.ClientID%>").value; 
            params.vRSoc = document.getElementById("<%=txtNombre.ClientID%>").value; 
            params.vContac = document.getElementById("<%=txtContacto.ClientID%>").value; 
            params.vCel = document.getElementById("<%=txtCelular.ClientID%>").value; 
            params.vDir = document.getElementById("<%=txtDireccion.ClientID%>").value; 
            params.vMail = document.getElementById("<%=txtMail.ClientID%>").value; 
            params.vUser = "<%=Session["idUser2"]%>";
			params = JSON.stringify(params);

			$.ajax({
                type: "POST", url: "MantProveedor.aspx/SetBtnGuardar", data: params, 
		...
		function btnModificar() {
			if (document.getElementById("<%=lblCodProv.ClientID%>").innerText == "") {
				alert("Seleccione un Producto.");
				return;
			}

			var params = new Object();
            params.vCodigo = document.getElementById("<%=lblCodProv.ClientID%>").innerText; 
            params.vNDoc = document.getElementById("<%=txtRUC.ClientID%>").value; 
            params.vRSoc = document.getElementById("<%=txtNombre.ClientID%>").value; 
            params.vContac = document.getElementById("<%=txtContacto.ClientID%>").value; 
            params.vCel = document.getElementById("<%=txtCelular.ClientID%>").value; 
            ...
            $.ajax({
                type: "POST", url: "MantProveedor.aspx/SetBtnModificar", data: params, 
                contentType: "application/json; charset=utf-8", dataType: "json", async: true, 
                success: function (result) { $("#divMensaje").html(result.d) }, 
                error: function(XMLHttpRequest, textStatus, errorThrown) { 
                    alert(textStatus + ": " + XMLHttpRequest.responseText); 
		...
		function BorraReg(vcod) {
            ...
            $.ajax({
                type: "POST", url: "MantProveedor.aspx/SetBtnEliminar", data: params, 
                contentType: "application/json; charset=utf-8", dataType: "json", async: true, 
...
<asp:Content ID="Content3" ContentPlaceHolderID="ContentPlaceHolder3" Runat="Server">
    <div class="col-xs-8 col-xs-offset-2">
        <p class="cazador2">Mantenimiento de Proveedor</p>
    </div>
</asp:Content>
...
function btnCancelar()
{
	...
	document.getElementById("<%=lblCodProv.ClientID%>").innerText = "";
				...
				<div class="column">
					<div class="col-md-8" style="padding:  0px 30px 10px 30px; ">
						...
						<div class="row">
							...
							<div class="col-md-2">
								<asp:Label ID="lblCodProv" runat="server" class='form-control'></asp:Label>
							</div>
						</div>
						<p></p>
						<div class="row">
						...
						<div class="row">
							...
							<div class="col-md-3">
								<asp:TextBox ID="txtCelular" runat="server" class='form-control' placeholder="Celular"></asp:TextBox>
								<code style="color:black; background-color:transparent">Celular</code>
							</div>
						</div>
						<p></p>
						<div class="row">
							<div class="col-md-6">
								...
								<code style="color:black; background-color:transparent">Direccion</code>
							</div>
							<div class="col-md-6">
								<asp:TextBox ID="txtMail" runat="server" class='form-control'></asp:TextBox>
								<code style="color:black; background-color:transparent">e-mail</code>
							</div>
						</div>
      ...
      if (event.keyCode === 13) {
		  ...
		  document.getElementById("<%=bntBuscar.ClientID%>").click();
            ...
            <div class="input-group margin">
                            ...
                            <div><input type="text" class="form-control" id="bscprod2" placeholder="Buscar Nombre de Proveedor" onkeyup="fBscTblHTML('bscprod2', 'tblbscrJS', 1)" autofocus="autofocus">
                            </div>
            </div>
                            </h3>
...
<script>
	...
	function SelFilatbl(vcod, vruc, vrsoc, vcont, vcel, vdir, vmail)
	{
        document.getElementById("<%=lblCodProv.ClientID%>").innerText = vcod; 
        document.getElementById("<%=txtRUC.ClientID%>").value = vruc; 
        document.getElementById("<%=txtNombre.ClientID%>").value = vrsoc; 
        document.getElementById("<%=txtContacto.ClientID%>").value = vcont; 
        document.getElementById("<%=txtCelular.ClientID%>").value = vcel; 
        document.getElementById("<%=txtDireccion.ClientID%>").value = vdir; 
        document.getElementById("<%=txtMail.ClientID%>").value = vmail; 
    ...
    <script>
        ...
        document.getElementById("ulLiOptMantProv").classList.add('active');
        var f = new Date();
