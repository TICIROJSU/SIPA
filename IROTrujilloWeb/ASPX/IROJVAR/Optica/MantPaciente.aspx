<%@ Page Language="C#" MasterPageFile="../../MasterPage.master" AutoEventWireup="true" CodeFile="MantPaciente.aspx.cs" Inherits="ASPX_IROJVAR_Optica_MantPaciente" %>
   ...
   <title>IRO - HISMINSA</title>
    ...
    <script language="javascript" type="text/javascript">
		...
		function btnAgregar() {
            ...
            $.ajax({
                type: "POST", url: "MantPaciente.aspx/SetBtnGuardar", data: params, 
		...
		function btnModificar() {
			if (document.getElementById("<%=lblCodPac.ClientID%>").innerText == "") {
				alert("Seleccione un Paciente.");
				return;
			}
			if (document.getElementById("<%=lblAPP.ClientID%>").innerText == "SIPA") {
				btnModificarSIPA();
			}
			if (document.getElementById("<%=lblAPP.ClientID%>").innerText == "SOptica") {
				btnModificarSOptica();
			}
        }

		function btnModificarSIPA() {
            var params = new Object();
            params.vAPP = document.getElementById("<%=lblAPP.ClientID%>").innerText; 
            params.vCodigo = document.getElementById("<%=lblCodPac.ClientID%>").innerText; 
            params.vCelular = document.getElementById("<%=txtCelular.ClientID%>").value; 
            params.vDireccion = document.getElementById("<%=txtDireccion.ClientID%>").value; 
            params.vUser = "<%=Session["idUser2"]%>";
			params = JSON.stringify(params);

            $.ajax({
                type: "POST", url: "MantPaciente.aspx/SetBtnModificarSIPA", data: params, 
                contentType: "application/json; charset=utf-8", dataType: "json", async: true, 
                success: function (result) { $("#divMensaje").html(result.d) }, 
                error: function(XMLHttpRequest, textStatus, errorThrown) { 
                    alert(textStatus + ": " + XMLHttpRequest.responseText); 
		...
		function btnModificarSOptica() {
            ...
            params.vDireccion = document.getElementById("<%=txtDireccion.ClientID%>").value; 
            params.vUser = "<%=Session["idUser2"]%>";
            params.vApellidos = document.getElementById("<%=txtApellidos.ClientID%>").value; 
            params.vNombres = document.getElementById("<%=txtNombre.ClientID%>").value; 
            params.vDNI = document.getElementById("<%=txtDNI.ClientID%>").value; 
			params = JSON.stringify(params);

            $.ajax({
                type: "POST", url: "MantPaciente.aspx/SetBtnModificarOptica", data: params, 
                contentType: "application/json; charset=utf-8", dataType: "json", async: true, 
                success: function (result) { $("#divMensaje").html(result.d) }, 
                error: function(XMLHttpRequest, textStatus, errorThrown) { 
                    alert(textStatus + ": " + XMLHttpRequest.responseText); 
		...
		function BorraReg(vapp, vcod) {
			if (vapp == "SIPA") {
				document.getElementById("divMsg").innerText = "Paciente SIPA no puede Borrarse. Consulte con el Administrador";
            ...
            $.ajax({
                type: "POST", url: "MantPaciente.aspx/SetBtnEliminar", data: params, 
                contentType: "application/json; charset=utf-8", dataType: "json", async: true, 
		...
		function vermensaje() {
			if (document.getElementById("<%=lblAPP.ClientID%>").innerText == "SIPA") {
				document.getElementById("divMsg").innerText = "Solo se Modifica Celular y Direccion";
			}
			if (document.getElementById("<%=lblAPP.ClientID%>").innerText == "SOptica") {
				document.getElementById("divMsg").innerText = "Edicion Completa";
			}
...
<asp:Content ID="Content3" ContentPlaceHolderID="ContentPlaceHolder3" Runat="Server">
    <div class="col-xs-8 col-xs-offset-2">
        <p class="cazador2">Mantenimiento de Pacientes</p>
    </div>
</asp:Content>
...
function btnCancelar()
{
	...
	document.getElementById("<%=lblCodPac.ClientID%>").innerText = "";
				...
				<div class="column">
					<div class="col-md-8" style="padding:  0px 30px 10px 30px; ">
						...
						<div class="row">
							...
							<div class="col-md-2">
								<asp:Label ID="lblAPP" runat="server" class='form-control'></asp:Label>
							</div>
							<div class="col-md-2">
								<asp:Label ID="lblCodPac" runat="server" class='form-control'></asp:Label>
							</div>
						</div>
						<p></p>
						<div class="row">
						...
						<div class="row">
							...
							<div class="col-md-4">
								<asp:TextBox ID="txtDNI" runat="server" class='form-control' style="text-align: right"></asp:TextBox>
							</div>
							<div class="col-md-2">Celular</div>
							<div class="col-md-4">
								<asp:TextBox ID="txtCelular" runat="server" class='form-control' style="text-align: right"></asp:TextBox>
							</div>
						</div>
						<p></p>
						<div class="row">
						...
						<div class="row">
							...
							<div class="col-md-6">
								<div class="form-group has-warning">
									<label><i class="fa fa-bell-o"></i> <span id="divMsg"></span> </label>
								</div>
							</div>
...
<script>
	var inputText = document.getElementById("<%=txtBuscar.ClientID%>");
      ...
      if (event.keyCode === 13) {
		  ...
		  document.getElementById("<%=bntBuscar.ClientID%>").click();
            ...
            <div class="input-group margin">
                            ...
                            <div><input type="text" class="form-control" id="bscprod2" placeholder="Buscar Nombre de Paciente" onkeyup="fBscTblHTML('bscprod2', 'tblbscrJS', 1)" autofocus="autofocus">
                            </div>
            </div>
                            </h3>
...
<script>
	...
	function SelFilatbl(vapp, vcod, vape, vnom, vdni, vdir, vcel)
	{
        document.getElementById("<%=lblAPP.ClientID%>").innerText = vapp; 
        document.getElementById("<%=lblCodPac.ClientID%>").innerText = vcod; 
        document.getElementById("<%=txtApellidos.ClientID%>").value = vape; 
        document.getElementById("<%=txtNombre.ClientID%>").value = vnom; 
        document.getElementById("<%=txtDNI.ClientID%>").value = vdni; 
        document.getElementById("<%=txtDireccion.ClientID%>").value = vdir; 
        document.getElementById("<%=txtCelular.ClientID%>").value = vcel; 
		document.getElementById("upsubirpag").click();
		document.getElementById("divregistro").style.display = "block";
		vermensaje();
	}
    ...
    <script>
        ...
        document.getElementById("ulOpticaMant").classList.add('active');
        document.getElementById("ulLiOptMantPac").classList.add('active');
        var f = new Date();
